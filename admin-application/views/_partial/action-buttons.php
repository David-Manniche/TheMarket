...
if ((!isset($statusButtons) || true === $statusButtons)) {
    $div->appendElement('a', array('href' => 'javascript:void(0)', 'class' => 'btn-clean btn-sm btn-icon btn-secondary toolbar-btn-js d-none', 'title' => Labels::getLabel('LBL_Publish', $adminLangId), "onclick" => "toggleBulkStatues(1)"), '<i class="fas fa-eye"></i>', true);

    $div->appendElement('a', array('href' => 'javascript:void(0)', 'class' => 'btn-clean btn-sm btn-icon btn-secondary toolbar-btn-js d-none', 'title' => Labels::getLabel('LBL_Unpublish', $adminLangId), "onclick" => "toggleBulkStatues(0)"), '<i class="fas fa-eye-slash"></i>', true);
}
...
if (isset($otherButtons) && is_array($otherButtons)) {
    foreach ($otherButtons as $attr) {
        ...
        $div->appendElement('a', $attr['attr'], $attr['label'], true);
    }
}
