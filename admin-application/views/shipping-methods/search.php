...
foreach ($arr_listing as $sn => $row) {
    ...
    foreach ($arr_flds as $key => $val) {
        ...
        switch ($key) {
            ...
            case 'action':
                if ($canEdit) {
                    $td->appendElement('a', array('href'=>'javascript:void(0)','class'=>'btn btn-clean btn-icon','title'=>Labels::getLabel('LBL_Edit', $adminLangId),"onclick"=>"editForm(".$row['shippingapi_id'].")"), "<i class='ion-edit icon'></i>", true);
                    if ($row['shippingapi_id'] > 1) {
                        $td->appendElement('a', array('href'=>'javascript:void(0)','class'=>'btn btn-clean btn-icon','title'=>Labels::getLabel('LBL_Settings', $adminLangId),"onclick"=>"settingsForm(".$row['shippingapi_id'].")"), "<i class='ion-settings icon'></i>", true);
                    }
                }
                break;
            default:
