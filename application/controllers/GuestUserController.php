...
class GuestUserController extends MyAppController
{
    ...
    private function setLoginErrorMessage($message, $redirect = true)
    {
        if (true ===  MOBILE_APP_API_CALL) {
    ...
    private function redirectUser($preferredDashboard = 0, $redirect = true)
    {
        $redirectUrl = User::getPreferedDashbordRedirectUrl($preferredDashboard);
        $cartObj = new Cart();
        if ($cartObj->hasProducts()) {
            $redirectUrl = CommonHelper::generateFullUrl('cart');
            if (true === $redirect) {
    ...
    public function loginFacebook()
    {
        ...
        if ($userInfo['user_facebook_id'] != $userFacebookId) {
    ...
    public function loginGoogle()
    {
        ...
        if (isset($userGoogleEmail) && (!empty($userGoogleEmail))) {
            ...
            if ($userInfo['user_googleplus_id'] != $userGoogleId) {
                $message = Labels::getLabel("MSG_USER_SOCIAL_CREDENTIALS_NOT_MATCHED", $this->siteLangId);
                $this->setLoginErrorMessage($message);
            }
    ...
    public function loginApple()
    {
        ...
        if (isset($appleResponse['id_token'])) {
            ...
            if (!empty($userInfo)) {
                $userId = $userInfo['user_id'];
                $userObj = new User($userId);
                $arr = array('user_apple_id' => $userAppleId);
                if (!$userObj->setUserInfo($arr)) {
                    $message = Labels::getLabel($userObj->getError(), $this->siteLangId);
                    $this->setLoginErrorMessage($message, false);
                }
