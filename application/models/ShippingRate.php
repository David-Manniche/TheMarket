...
class ShippingRate extends MyAppModel
{
    ...
    public static function getSearchObject($langId)
    {
		...
		if ($langId > 0) {
            $srch->joinTable(static::DB_TBL_LANG,
                'LEFT OUTER JOIN',
                'ratelang.'.static::DB_TBL_LANG_PREFIX .'shiprate_id = srate.'.static::tblFld('id').' and ratelang.'.static::DB_TBL_LANG_PREFIX .'lang_id = '.$langId, 'ratelang');
        }
		return $srch;
    }
	...
	{
		return array(
			self::CONDITION_TYPE_WEIGHT => Labels::getLabel('LBL_Based_on_item_weight', $langId),
			self::CONDITION_TYPE_PRICE => Labels::getLabel('LBL_Based_on_item_price', $langId),
		);
	}
