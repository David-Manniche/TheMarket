...
<div class="sectionhead">
	<h5><?php echo Labels::getLabel('LBL_Zone_Setup', $siteLangId); ?>
	</h5>
</div>
<div class="sectionbody space">
	<form onsubmit="setupZone(this); return(false);" method="post" class="form" id="shippingZoneFrm">
		...
		<input type="hidden" name="shipzone_user_id"
			value="<?php echo $userId;?>">
		<input type="hidden" name="shipzone_id"
			value="<?php echo $zone_id;?>">
		<input type="hidden" name="shipzone_profile_id"
		...
		<div class="row">
			<div class="col-md-12">
				<div class="form-group mb-20 zone-main-field--js">
					<input type="text"
						placeholder="<?php echo Labels::getLabel("LBL_Zone_Name", $siteLangId); ?>"
						name="shipzone_name" class="form-control shipzone_name"
						value="<?php echo (!empty($zone_data))? $zone_data['shipzone_name']:'';?>"
						required>
		...
		<div class="row simplebar-resize-wrapper mb-20">
			<div class="col-sm-12">
				<div class="field-wraper mb-2">
					<div class="field_cover">
						<label>
							<span class="checkbox" data-zoneid="-1"><input type="checkbox" name="rest_of_the_world"
									value="-1" class="checkbox_zone_-1" <?php echo (in_array(-1, $zoneIds)) ?'checked':'';?>
								<?php echo (in_array(-1, $exZoneIds)) ?'disabled':'';?>><i
									class="input-helper"></i></span><?php echo Labels::getLabel("LBL_REST_OF_THE_WORLD", $siteLangId); ?>
						</label>
					</div>
				</div>
        ...
        $totalCountries = count($countries); ?>
					<div class="row no-gutters zone-row">
						<div class="col">
							<div class="field-wraper">
								<div class="field_cover">
									<label>
										<span class="checkbox zone--js"
											data-zoneid="<?php echo $zone['zone_id']; ?>"><input
												type="checkbox" name="shiploc_zone_ids[]"
						...
						<div class="col-auto">
							<a class="link font-bolder" data-toggle="collapse"
								href="#countries_list_<?php echo $zone['zone_id']; ?>"
								role="button" aria-expanded="false"
								aria-controls="countries_list_<?php echo $zone['zone_id']; ?>"><span
									class="fa fa-angle-down" aria-hidden="true"></span> </a>
						</div>
					</div>
					<?php if (!empty($countries)) { ?>
					<ul
						class="child-checkbox-ul zone_<?php echo $zone['zone_id'];?>">
            ...
            if (!empty($excludeCountryStates) && isset($excludeCountryStates[$countryId])) {
                $disabled = 'disabled';
						...
						<li class="collapse"
							id="countries_list_<?php echo $zone['zone_id']; ?>">
							<div class="row no-gutters">
								<div class="col">
									<div class="field-wraper">
										<div class="field_cover">
											<label>
												<span class="checkbox country--js "
													data-countryid="<?php echo $countryId; ?>"
													data-statecount="<?php echo $statesCount; ?>"><input
														type="checkbox" name="shiploc_country_ids[]"
														value="<?php echo $zone['zone_id']; ?>-<?php echo $countryId; ?>"
														class="checkbox_country_<?php echo $countryId; ?>"
														<?php echo $checked; ?>
													<?php echo $disabled; ?>><i
														class="input-helper"></i></span><?php echo $country['country_identifier']; ?>
											</label>
										</div>
									</div>
								</div>
								...
								<div class="col-auto mr-3">
									...
									<a class="link font-bolder link_<?php echo $countryId;?>"
										data-toggle="collapse"
										href="#state_list_<?php echo $countryId;?>"
										role="button" aria-expanded="false"
										aria-controls="state_list_<?php echo $countryId;?>"
										data-countryid="<?php echo $countryId;?>"
										data-loadedstates="0"
										onclick="getStates(<?php echo $countryId .','. $zone['zone_id'].','. $profile_id;?>);"><span
											class="statecount--js selectedStateCount--js_<?php echo $countryId;?> "
											data-totalcount="<?php echo $statesCount; ?>">0</span>
							...
							<div class="collapse"
								id="state_list_<?php echo $countryId; ?>">
							</div>
						</li>
						<?php
        } ?>
		...
		<div class="row">
			<div class="col-md-12 mt-4">
				<input type="submit" name="btn_submit"
					value="<?php echo Labels::getLabel("LBL_Add_Zone", $siteLangId); ?>">
				<input type="button" name="cancel"
					onClick="searchProductsSection(<?php echo $profile_id;?>);"
					value="<?php echo Labels::getLabel("LBL_Cancel", $siteLangId); ?>">
			</div>
		</div>
	</form>
</div>
