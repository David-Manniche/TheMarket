    ...
    foreach ($addresses as $address) {
        $address['addr_title'] = ($address['addr_title'] == '') ? '&nbsp;' : $address['addr_title']; ?> <div class="col-lg-4 col-md-6 mb-4 mb-md-0">
            <label class="address-block <?php echo ($address['addr_is_default']==1)?'is-active':''; ?>">
                ...
                <div class="btn-group"><a href="javascript:void(0)" onClick="addAddressForm(<?php echo $address['addr_id']; ?>)" class="btn btn-outline-brand btn-sm"><?php echo Labels::getLabel('LBL_Edit', $siteLangId); ?></a>
                <a href="javascript:void(0)" onClick="removeAddress(<?php echo $address['addr_id']; ?>)" class="btn btn-outline-brand btn-sm"><?php echo Labels::getLabel('LBL_Delete', $siteLangId); ?></a></div>
            </label>
        </div> <?php
    }
