        ...
        foreach ($arr_flds as $key => $val) {
            ...
            switch ($key) {
                ...
                case 'action':
                    ...
                    if ($order['op_product_type'] == Product::PRODUCT_TYPE_PHYSICAL && $shipBySeller && true === $canShipByPlugin && ('CashOnDelivery' == $order['plugin_code'] || Orders::ORDER_PAYMENT_PAID == $order['order_payment_status']) && !empty($row['opshipping_carrier_code']) && !empty($row['opshipping_service_code'])) {
                        $li = $ul->appendElement("li");
