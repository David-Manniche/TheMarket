    ...
    <ul class="arrowTabs">
        ...
        <?php foreach ($splitPaymentMethodsPlugins as $plugin) { ?>
            <li class="<?php echo !empty($action) && ($action == $plugin['plugin_code']) ? 'is-active' : ''; ?>">
                <a href="javascript:void(0)" class="pluginPlatform-js" <?php if ($shop_id > 0) { ?> onClick="pluginPlatform(this)" <?php } ?> data-platformurl="<?php echo CommonHelper::generateUrl($plugin['plugin_code'])?>">
