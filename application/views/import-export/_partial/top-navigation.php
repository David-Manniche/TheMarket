...
<div class="tabs tabs--small   tabs--scroll clearfix">
    <ul class="arrowTabs">
        <li class="<?php echo !empty($action) && $action == 'generalInstructions' ? 'is-active' : '';?>">
            <a href="javascript:void(0)" onClick="loadForm('general_instructions')">
                <?php echo Labels::getLabel('LBL_Instructions', $siteLangId); ?>
            </a>
        </li>
        <li class="<?php echo !empty($action) && $action == 'export' ? 'is-active' : '';?>">
            <a href="javascript:void(0)" onClick="loadForm('export')">
                <?php echo Labels::getLabel('LBL_Export', $siteLangId); ?>
            </a>
        </li>
        <li class="<?php echo !empty($action) && $action == 'import' ? 'is-active' : '';?>">
            <a href="javascript:void(0)" onClick="loadForm('import')">
                <?php echo Labels::getLabel('LBL_Import', $siteLangId); ?>
            </a>
        </li>
        <li class="<?php echo !empty($action) && $action == 'settings' ? 'is-active' : '';?>">
            <a href="javascript:void(0)" onClick="loadForm('settings')">
